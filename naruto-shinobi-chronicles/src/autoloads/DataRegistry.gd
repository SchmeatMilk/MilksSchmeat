...
func load_all(base: String) -> bool:
	...
	# Visual manifests (optional — fall back to palette blocks if absent).
...
func _read_json_opt(path: String) -> Dictionary:
...
func unit_visual(id: String) -> Dictionary:
	return unit_visuals.get(id, {})


func building_visual(type: String) -> Dictionary:
	return building_visuals.get(type, {})
