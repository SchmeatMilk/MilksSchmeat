...
func _ready() -> void:
	...
	_build_ui()
	# A battle-return dialogue (if any) plays first; on_enter cutscenes fire either
...
## Returns true if a battle-return dialogue was opened (so the caller defers the
...
func _interact() -> void:
	...
	# Conditional dialogue: the first matching state's lines, else the base lines.
...
## Apply a resolved NPC's effects then its primary action.
...
func _on_cutscene_finished() -> void:
	...
	if _active_cutscene_id != "":
		...
		_active_cutscene_id = ""
	_apply_effects(fin)
	# Rebuild NPCs so recruits/flag changes are reflected immediately.
	for child in npc_nodes.values():
		child.queue_free()
	npc_nodes = {}
	_build_npcs()
